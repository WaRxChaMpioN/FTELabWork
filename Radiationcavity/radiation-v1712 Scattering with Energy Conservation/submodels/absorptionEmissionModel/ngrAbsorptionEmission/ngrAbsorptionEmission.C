...
#include "ngrAbsorptionEmission.H"
...
namespace Foam
{
	...
	defineTypeNameAndDebug(ngrAbsorptionEmission, 0);
...
Foam::radiation::ngrAbsorptionEmission::aCont
(
...
) const
{
	...
	int iter=0;int iterprev=0; int iterpost=1;int iterprevP=0;int iterpostP=1;
...
/*	int m=0;
    ...
    iter=1; iterprev=1; iterpost=1;iterprevP=1;iterpostP=1;
    ...
    iterprev=itprev[celli]+nts*nMoleFractions*nWeights_*(indiceP[0]);

    iterpost=itpost[celli]+nts*nMoleFractions*nWeights_*(indiceP[0]);

    iterprevP=itprev[celli]+nts*nMoleFractions*nWeights_*(indiceP[1]);        
    
    iterpostP=itpost[celli]+nts*nMoleFractions*nWeights_*(indiceP[1]);  

    
    a[celli]=(absCoeffs_[iterprev]+(1-weights[0])*(absCoeffs_[iterpost]-absCoeffs_[iterprev]));//+(1-weightP[1])*(absCoeffs_[iterprevP]+(1-weights[0])*(absCoeffs_[iterpostP]-absCoeffs_[iterprevP]));
...
Foam::tmp<Foam::volScalarField>
Foam::radiation::ngrAbsorptionEmission::eCont
(
    const label bandI
) const
{
...
void Foam::radiation::ngrAbsorptionEmission::correct
(
    volScalarField& a,
