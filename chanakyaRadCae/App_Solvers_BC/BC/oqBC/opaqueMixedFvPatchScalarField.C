...
#include "opaqueMixedFvPatchScalarField.H"
...
opaqueMixedFvPatchScalarField
(
    const opaqueMixedFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
void Foam::radiation::opaqueMixedFvPatchScalarField::
updateCoeffs()
{
    if (this->updated())
    {
   ...
   // Info<<"the patch which i am are"<<patch().name()<<endl; 
   // Info<<"the sum of Ir values are"<<Ir<<endl; 
  // scalarField& qout = ray.qem().boundaryFieldRef()[patchi];
   ...
   // scalarField qem(n.size()); //if we do this type of initialization no need to divide total qem/2.0
   // scalarField qin(n.size());
    ...
    forAll(Iw, faceI)
    {
        if ((-n[faceI] & myRayId) > 0.0)
        {
         ...
         //  qout[faceI]= Iw[faceI]*nAve[faceI];
           //Info<<"the qem values are"<<qem[faceI]<<endl;
        }
        else
        {
  ...
  //   Info<<"the qem values are"<<qem<<endl;
    // Info<<"the qout values are"<<qout<<endl;
    // Restore tag
...
void Foam::radiation::opaqueMixedFvPatchScalarField::write
(
    Ostream& os
) const
{
