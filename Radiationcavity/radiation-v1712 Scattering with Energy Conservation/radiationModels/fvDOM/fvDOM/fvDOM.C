...
:
    ...
    nRay_(0),
    nLambda_(readLabel(coeffs_.lookup("nWeights"))), //Uncomment it when not using the LBL case
    //nLambda_(absorptionEmission_->nBands()), // Uncomment it when using the LBL case
    aLambda_(nLambda_),
...
:
    ...
    nRay_(0),
    nLambda_(readLabel(coeffs_.lookup("nWeights"))), //Uncomment it when not using the LBL case
    //nLambda_(readLabel(coeffs_.lookup("nBand"))), // Uncomment it when using the LBL case
    aLambda_(nLambda_),
    blackBody_(nLambda_, T),
    IRay_(0),
    convergence_(coeffs_.lookupOrDefault<scalar>("convergence", 0.0)),
...
void Foam::radiation::fvDOM::calculate()
{
    ...
    do
    {
        ...
        forAll(IRay_, rayI)
        {
            I_Sum = dimensionedScalar("zero",dimMass/pow3(dimTime), 0.0);
...
void Foam::radiation::fvDOM::qr_Domain()
{
...
void Foam::radiation::fvDOM::qr_Boundary()
{  
...
void Foam::radiation::fvDOM::energyCheck()
{
...
void Foam::radiation::fvDOM::qr_Cell()
{
   forAll(mesh_.cells(),celli)  //for(label celli = 0; celli<mesh_.nCells(); celli++)  
   {   
...
void Foam::radiation::fvDOM::delqr()
{
...
Foam::label Foam::radiation::fvDOM::nBands() const
{
